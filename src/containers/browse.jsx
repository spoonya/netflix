...
export default function BrowseContainer(props) {
  ...
  useEffect(() => {
    ...
    const res = fuse.search(searchTerm).map(({ item }) => item);

    if (slideRows.length > 0 && searchTerm.length > 3 && res.length > 0) {
      setSlideRows(res);
    } else {
      setSlideRows(slides[category]);
    }
      ...
      <Header src="joker1">
        ...
        <Header.Feature>
          ...
          <Header.Text>
            ...
            world around him. Isolated, bullied and disregarded by society,
            Fleck begins a slow descent into madness as he transforms into the
